...
function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName) {
			...
			if ($arProperty["USER_TYPE_SETTINGS"]){
				...
				foreach($arProperty["USER_TYPE_SETTINGS"] as $property){
					...
					$html.= "<div id='London_SetLine'><div id='London_title'>".$property["NAME"]."</div>";
					ob_start();
					...
					$html.= "<div id='London_table'>".$input."</div>";
					$html.= "</div>";
				}
			}
			$html.= "<style>
   ...
   function GetSettingsHTML($arProperty, $strHTMLControlName, &$arPropertyFields)
	{
		...
		$arNotSupportTypes = array("L","F","S:video","S:HTML");
		...
		foreach ($arProperty["USER_TYPE_SETTINGS"] as $key=>$property){
			...
			foreach ($arTypes as $type){
				...
				$typesOption .= "<option $selected  value='$type[VALUE]'>$type[NAME]</option>";
			}
