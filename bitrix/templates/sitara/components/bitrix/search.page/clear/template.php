			...
			<div class="search-advanced">
				<div class="search-advanced-result">
					<? if (is_object($arResult["NAV_RESULT"])): ?>
						<div
							class="search-result"><? echo GetMessage("CT_BSP_FOUND") ?>: <? echo $arResult["NAV_RESULT"]->SelectedRowsCount() ?></div>
					<? endif; ?>
					<?
						$arWhere = array();
				...
				<div class="search-advanced-filter"><a href="#"
						onclick="return switch_search_params()"><? echo GetMessage('CT_BSP_ADVANCED_SEARCH') ?></a>
				</div>
			</div><? //div class="search-advanced"?>
			<div id="search_params" class="search-filter"
				style="display:<? echo $arResult["REQUEST"]["FROM"] || $arResult["REQUEST"]["TO"] || $arResult["REQUEST"]["WHERE"] ? 'block' : 'none' ?>">
				...
				<table class="search-filter" cellspacing="0">
					...
					<tr>
						...
						<td class="search-filter-field"><input class="search-button"
								value="<? echo GetMessage("CT_BSP_GO") ?>" type="submit"></td>
					</tr>
					</tbody>
				</table>
			</div>
					...
					<? foreach ($arResult["SEARCH"] as $arItem): ?>
						<li id="<?= $this->GetEditAreaId($arItem['ID'] . "_" . $q); ?>">
							<div class="news-rigth">
								<span class='this-title'><a href="<? echo $arItem["URL"] ?>"><? echo
										$arItem["TITLE_FORMATED"] ?></a></span>

								<div class="description"><? echo html_entity_decode($arItem["BODY_FORMATED"]) ?></div>
							</div>
