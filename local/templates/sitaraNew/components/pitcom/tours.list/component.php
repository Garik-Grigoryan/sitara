	...
	if ($this->StartResultCache(false, array($arNavigation))) {
		...
		while ($obElement = $rsElement->GetNextElement()) {
			...
			if ($arElement["DETAIL_PICTURE"]) {
				$arElement["ALT"] = CFile::GetFileArray($arElement["DETAIL_PICTURE"]);

				$arElement["DETAIL_PICTURE"] = CFile::ResizeImageGet($arElement["DETAIL_PICTURE"], array("width" => $arParams['WIDTH'], "height" => $arParams['HEITH']), BX_RESIZE_IMAGE_EXACT, true);
			}
